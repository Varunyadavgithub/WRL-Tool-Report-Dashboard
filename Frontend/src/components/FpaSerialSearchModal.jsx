...
import { closeSerialModal, openDefectModal } from "../redux/fpaReportSlice";
import FpaDefectDetailModal from "./FpaDefectDetailModal";
import { IoCloseOutline } from "react-icons/io5";
import { FaArrowRight, FaBarcode } from "react-icons/fa";
import { HiOutlineClipboardList } from "react-icons/hi";

const FpaSerialSearchModal = () => {
  const dispatch = useDispatch();
  const { serialData, isDefectModalOpen, selectedFGSRNo } = useSelector(
    (state) => state.fpaReport,
  );

  const handleClose = () => dispatch(closeSerialModal());
  const handleDefectClick = (fgsrNo) => dispatch(openDefectModal(fgsrNo));

  if (!serialData) return null;

  const { modelName, data: records } = serialData;
  ...
  return (
    <>
        ...
        >
          ...
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                <FaBarcode className="text-blue-600" size={16} />
          ...
          {records.length > 0 && (
            <div className="px-6 py-3 bg-gray-50 border-b border-gray-200 flex flex-wrap gap-6 text-sm">
              <div>
                <span className="text-gray-500">FGSRNo: </span>
                  ...
                  {records.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={() => handleDefectClick(item.FGSRNo)}
      ...
      {/* Defect Detail Modal — opens on top */}
      {isDefectModalOpen && selectedFGSRNo && <FpaDefectDetailModal />}
    </>
  );
};

export default FpaSerialSearchModal;