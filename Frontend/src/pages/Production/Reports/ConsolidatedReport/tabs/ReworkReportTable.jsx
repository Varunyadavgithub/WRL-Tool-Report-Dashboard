...
function InfoCard({ icon: Icon, label, value, cls }) {
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-xl bg-white border border-slate-100 px-3.5 py-2.5 shadow-sm">
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${cls}`}
      >
      ...
      <div>
        ...
        <p
          className="text-sm font-bold text-slate-700 truncate max-w-[180px]"
          title={value}
        >
          {value}
        </p>
...
function ReworkReportTable({ data }) {
  if (!Array.isArray(data))
    return <EmptyState message="No rework data found." />;
            ...
            {data.length > 0 ? (
              data.map((item, idx) => {
                ...
                return (
                  <tr
                    key={idx}
                    className={`text-center transition-colors hover:bg-amber-50/30 ${bg}`}
                  >
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap text-slate-600">
                      {fmt(item.reworkIN) || (
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap">
                      <DurationBadge
                        duration={item.duration || item["Duration (DD,HH:MM)"]}
                      />
                    </td>
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap">
                      {item.defectCategory ? (
                        <span className="px-2.5 py-0.5 text-[11px] font-semibold rounded-full bg-violet-50 text-violet-700 border border-violet-200">
                          {item.defectCategory}
                        </span>
                      ) : (
                        <span className="text-slate-300 italic">—</span>
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap max-w-[180px]">
                      {item.defect ? (
                        <span
                          className="inline-flex items-center gap-1 px-2.5 py-0.5 text-[11px] font-semibold rounded-full bg-red-50 text-red-600 border border-red-200"
                          title={item.defect}
                        >
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap max-w-[160px]">
                      {item.rootCause ? (
                        <span
                          className="bg-orange-50 text-orange-700 px-2 py-0.5 rounded text-[11px] font-semibold border border-orange-200"
                    ...
                    <td className="px-3 py-2.5 border-b border-slate-100 whitespace-nowrap max-w-[160px]">
                      {item.counterAction ? (
                        <span
                          className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-[11px] font-semibold border border-blue-200"
              ...
              <tr>
                <td colSpan={headers.length} className="py-16 text-center">
                  <div className="flex flex-col items-center gap-3 text-slate-400">
                    ...
                    />
                    <p className="text-sm">
                      No rework data found for this serial number.
                    </p>
                  </div>
                </td>
              </tr>
