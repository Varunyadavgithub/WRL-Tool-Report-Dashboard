...
const oeeBarColor = (v) =>
  v >= 85 ? "bg-emerald-500" : v >= 65 ? "bg-amber-500" : "bg-rose-500";
...
const aggregateRecords = (
  ...
  enriched.forEach((r) => {
    ...
    if (!map[key]) {
      map[key] = {
        ...
        dateFrom: day,
        dateTo: day,
        startedAtMs: r._absMs ?? null,
        completedAtMs: r._absMsEnd ?? r._absMs ?? null,
        startedAt: r.startTime || "",
        completedAt: r.endTime || r.startTime || "",
        planQty: 0,
        actualQty: 0,
        goodQty: 0,
    ...
    if (
      r._absMs != null &&
      (g.startedAtMs == null || r._absMs < g.startedAtMs)
    ) {
    ...
    if (
      recordEndMs != null &&
      (g.completedAtMs == null || recordEndMs > g.completedAtMs)
    ) {
  ...
  const availablePlans = plans
    ...
    .filter(
      (p) =>
        (!queryDateFrom || p.planDate >= queryDateFrom) &&
        (!queryDateTo || p.planDate <= queryDateTo),
    )
    .map((p) => ({ ...p, _used: false }));

  const consumePlan = (day, shift, sapCode) => {
    let p = availablePlans.find(
      (c) =>
        !c._used &&
        c.sapCode === sapCode &&
        c.planDate === day &&
  ...
  const producedRows = Object.values(map)
    ...
    .map((row, idx) => {
      const avgCycleSecs =
        row.cycleSecs.length > 0
          ? Math.round(
              row.cycleSecs.reduce((a, b) => a + b, 0) / row.cycleSecs.length,
            )
      ...
      const compCT =
        punching && compPerSheet > 0 && noOfSheet > 0
          ? Math.round(
              ((avgCycleSecs + loadUnload) / (compPerSheet * noOfSheet)) * 100,
            ) / 100
          : null;

      // Total Components Produced = (sheets × comps/sheet) × machine sheet count
      const compQty =
        punching && noOfSheet > 0 && compPerSheet > 0
      ...
      const compMultiplier =
        compQty > 0 && row.actualQty > 0 ? compQty / row.actualQty : 1;
      const fallbackRejects = Math.round(
        (row.actualQty - row.goodQty) * compMultiplier,
      );
      const logRejects =
        qualityByPartName[row.itemDescription ?? ""]?.rejected ??
        qualityByPartName[
          getMaterialByModel(materials, row.model)?.partName ?? ""
        ]?.rejected;
      ...
      const A =
        availableTimeSecs > 0
          ? Math.max(
              ...
              Math.min(
                1,
                (availableTimeSecs - totalDowntimeSecs) / availableTimeSecs,
              ),
            )
          : 0;
      // Performance uses ideal production time divided by the net operating
      // time, matching the Dashboard OEE calculation.
      const netOperatingSecs = Math.max(
        1,
        availableTimeSecs - totalDowntimeSecs,
      );
      const P =
        row.definedCycleTime > 0
          ? Math.max(
              0,
              Math.min(1, (compQty * row.definedCycleTime) / netOperatingSecs),
            )
          : 1;
      ...
      const oee =
        Math.round(((availability * performance * quality) / 10000) * 10) / 10;

      const goodComp = accepted;

      // Energy
      const idealEnergyWh = Math.round(
        (planQty * row.definedCycleTime * MACHINE_POWER_KW) / 3600,
      );
      const actualEnergyWh = Math.round(
        (row.actualQty * avgCycleSecs * MACHINE_POWER_KW) / 3600,
      );
  ...
  const ghostRows = availablePlans
    ...
    .map((p, i) => ({
      ...
      loadUnload: 0,
      stdChangeoverTime: STD_CHANGEOVER_MINS,
      actualChangeoverTime: 0,
      plannedChangeovers: 0,
      actualChangeovers: 0,
      coOverrunMins: 0,
      coOverrunCount: 0,
      idleMins: 0,
...
const StatCard = ({
  ...
  return (
    <div
      className={`relative flex flex-col rounded-xl border ${t.card} px-3.5 py-3 overflow-hidden
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span
          className={`text-[10px] font-semibold uppercase tracking-wide ${t.sub}`}
        >
          {label}
        </span>
      ...
      <div className="flex items-end gap-2 mt-1">
        <span
          className={`font-mono font-bold ${primary ? "text-2xl" : "text-lg"} ${t.text}`}
        >
          {value}
        </span>
        {delta != null && (
          <span
            className={`text-[10px] font-bold mb-1 flex items-center gap-0.5 ${delta >= 0 ? "text-emerald-600" : "text-rose-500"}`}
          >
            {delta >= 0 ? (
...
const Delta = ({ actual, plan }) => {
  ...
  return (
    <span
      className={`text-[9px] font-bold ${diff >= 0 ? "text-emerald-600" : "text-rose-500"}`}
    >
      {diff >= 0 ? "▲" : "▼"}
...
const buildColumns = (materials, showFormula) => [
  {
    ...
    align: "left",
    cell: (r) => (
      <span className="text-[11px] text-slate-400 font-mono">{r.srNo}</span>
    ),
    csv: (r) => r.srNo,
  },
  {
  ...
  },
  {
    ...
    cell: (r) =>
      materials.some((m) => m.partName === r.itemDescription) ? (
        <span className="font-semibold text-blue-700 text-xs leading-snug block max-w-[240px]">
          {r.itemDescription}
        </span>
      ) : (
        <div className="max-w-[240px]">
          <span className="font-mono text-[11px] text-slate-600 block leading-snug">
            {r.itemDescription}
          </span>
          <span className="text-[9px] font-bold text-rose-500 flex items-center gap-0.5">
            <FiAlertTriangle className="w-2.5 h-2.5" /> Master not found
          </span>
        </div>
      ),
  ...
  },
  {
    ...
    wide: true,
    cell: (r) => (
      <span className="text-[11px] text-slate-500 leading-snug block max-w-[200px]">
        {r.sheetDescription || "—"}
      </span>
    ),
    ...
    cell: (r) => (
      <div className="text-center">
        <span className="font-mono font-semibold text-slate-700">
          {r.planQty.toLocaleString()}
        </span>
        {showFormula && (
          <div className="text-[9px] font-mono text-slate-400">
            {r.planQtyFromConfig
              ? "from Planning Config"
              : `${r.componentQty}×1.05 (estimated)`}
          </div>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    cell: (r) => (
      <div className="flex flex-col items-center gap-0.5">
        <span
          className={`font-bold font-mono text-sm ${r.componentQty >= r.planQty ? "text-emerald-600" : r.isPunching ? "text-violet-600" : "text-blue-500"}`}
        >
        ...
        <div className="w-12 h-1 bg-slate-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${r.componentQty >= r.planQty ? "bg-emerald-500" : "bg-blue-500"}`}
        ...
        {showFormula &&
          ...
          r.compPerSheet > 0 && (
            <div className="text-[9px] font-mono text-slate-400">
              {r.noOfSheet}×{r.compPerSheet}×{r.actualQty}
            </div>
          )}
      </div>
    ),
  ...
  },
  {
    ...
    align: "center",
    cell: (r) => (
      <span className="font-bold font-mono text-sm text-slate-600">
  ...
  },
  {
    ...
    cell: (r) => (
      <span
        className={`font-mono font-semibold ${r.actualTimeMins > r.reqTimeMins ? "text-amber-600" : "text-violet-600"}`}
      >
        {r.actualTimeMins}
      </span>
    ),
  ...
  },
  {
    ...
    cell: (r) => (
      <span className="font-mono font-semibold text-rose-600">
        {r.totalDowntimeMins} min
      </span>
    ),
    csv: (r) => r.totalDowntimeMins,
  },
  {
    key: "definedCycleTime",
    label: "Standard CT",
    group: "ct",
    sortable: true,
    align: "center",
  ...
  },
  {
    ...
    cell: (r) => {
      ...
      return (
        <div className="text-center">
          <span
            className={`font-mono font-bold ${good ? "text-emerald-600" : "text-amber-600"}`}
          >
            {r.sheetCycleTime > 0 ? r.sheetCycleTime : "-"}
          </span>
          {showFormula && r.isPunching && r.noOfSheet > 0 && (
            <div className="text-[9px] font-mono text-slate-400">
              {r.machineCycleSecs} ÷ {r.noOfSheet}
            </div>
          )}
        </div>
      );
      ...
      r.componentCycleTime != null ? (
        <div className="text-center">
          <span className="font-mono font-semibold text-indigo-600">
            {r.componentCycleTime}
          </span>
          {showFormula && (
            <div className="text-[9px] font-mono text-slate-400">
              ({r.machineCycleSecs}+{r.loadUnload}) ÷ ({r.compPerSheet}×
              {r.noOfSheet})
            </div>
          )}
        </div>
      ...
      <div className="text-center font-mono font-semibold text-slate-600">
        ...
        {showFormula && (
          <div className="text-[9px] font-mono text-slate-400 whitespace-nowrap">
            ({r.reqTimeMins} - {r.totalDowntimeMins}) / {r.reqTimeMins} x 100
          </div>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    cell: (r) => (
      <div className="text-center font-mono font-semibold text-slate-600">
        ...
        {showFormula && (
          <div className="text-[9px] font-mono text-slate-400 whitespace-nowrap">
            ({r.componentQty} x {r.definedCycleTime}) / (({r.reqTimeMins} -{" "}
            {r.totalDowntimeMins}) x 60) x 100
          </div>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    cell: (r) => (
      <div className="text-center font-mono font-semibold text-slate-600">
        ...
        {showFormula && (
          <div className="text-[9px] font-mono text-slate-400 whitespace-nowrap">
            {r.accepted} / {r.componentQty} x 100
          </div>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    cell: (r) => (
      <div className="text-center">
        <div
          className={`inline-flex flex-col items-center px-2.5 py-1 rounded-lg border ${oeeBgColor(r.oee)}`}
        >
          <span
            className={`text-sm font-bold font-mono ${oeeTextColor(r.oee)}`}
          >
            {r.oee}%
          </span>
          <div className="w-10 h-1 bg-white/60 rounded-full overflow-hidden mt-0.5">
            <div
              className={`h-full rounded-full ${oeeBarColor(r.oee)}`}
              style={{ width: `${r.oee}%` }}
            />
          </div>
        </div>
        {showFormula && (
          <div className="text-[9px] font-mono text-slate-400 mt-0.5 whitespace-nowrap">
            {r.availability} x {r.performance} x {r.quality} / 10,000
          </div>
        )}
      </div>
    ),
...
const computeTotals = (rows, columns) => {
  const out = {};
  columns.forEach((c) => {
    if (AGG_SUM.has(c.key))
      out[c.key] = rows.reduce((s, r) => s + (Number(c.csv(r)) || 0), 0);
    else if (AGG_AVG.has(c.key))
      out[c.key] = rows.length
        ? rows.reduce((s, r) => s + (Number(c.csv(r)) || 0), 0) / rows.length
        : 0;
    else out[c.key] = null;
  });
  return out;
...
const exportCSV = (rows, columns) => {
  ...
  const body = rows
    .map((r) => cols.map((c) => csvEscape(c.csv(r))).join(","))
    .join("\n");
  downloadBlob(
    header + "\n" + body,
...
const exportExcel = async (rows, columns, meta) => {
  ...
  const groupTh = (s) =>
    `<th colspan="${s.count}" style="background:#e2e8f0;color:#1e293b;font-weight:bold;border:1px solid #1e293b;padding:5px;text-align:center">${(GROUP_META[s.group] || GROUP_META.info).label}</th>`;
  const th = (c) =>
    `<th style="background:#1e3a8a;color:#ffffff;font-weight:bold;border:1px solid #1e293b;padding:6px;text-align:${c.align === "center" ? "center" : "left"}">${c.label}</th>`;
  const td = (c, v) => {
  ...
  const body = rows
    .map(
      (r, i) =>
        `<tr style="background:${i % 2 ? "#f1f5f9" : "#ffffff"}">${cols.map((c) => td(c, c.csv(r))).join("")}</tr>`,
    )
    .join("");
  const foot = `<tr>${cols
    .map(
      (c, i) =>
        `<td style="background:#dbeafe;font-weight:bold;border:1px solid #1e293b;padding:5px;text-align:${c.align === "center" ? "center" : "left"}">${fmtTotal(c, totals, i === 0)}</td>`,
    )
    .join("")}</tr>`;
  const logoCell = logo
    ? `<td style="width:90px;padding:6px"><img src="${logo}" width="80" height="40" /></td>`
    : "";
...
const exportPDF = async (rows, columns, meta) => {
  ...
  doc.text(
    `${meta.start} to ${meta.end}   |   Generated ${meta.generated}   |   ${rows.length} parts`,
    titleX,
    42,
  );
  doc.setTextColor(0);
  ...
  const footRow = cols.map((c, i) => ({
    ...
    styles: {
      halign: c.align === "center" ? "center" : "left",
      fontStyle: "bold",
    },
  }));
  const columnStyles = {};
  cols.forEach((c, i) => {
    columnStyles[i] = { halign: c.align === "center" ? "center" : "left" };
  });
...
const PartProcessProductionReport = () => {
  ...
  const fetchData = useCallback(async (start, end, setLoadFn) => {
    ...
    try {
      ...
      const last = new Date(endMs - 1000);
      ...
      for (const date of dates) {
        const raw = allRows.filter(
          (r) => String(r.EventDate).slice(0, 10) === date,
        );
        raw.forEach((r) => {
        ...
        const mapped = raw
          .map((r, i) => ({ ...mapDbRecord(r, i), eventDate: date }))
          .map((r) => {
      ...
      toast.success(
        `${prodCount} production + ${filtered.length - prodCount} downtime records loaded`,
      );
    } catch (err) {
      setRecords([]);
      setDbQLogs([]);
      toast.error(
        err?.response?.data?.message || "Failed to fetch production records.",
      );
    } finally {
      setLoadFn(false);
    }
  ...
  const columns = useMemo(
    () => buildColumns(materials, showFormula),
    [materials, showFormula],
  );
  ...
  useEffect(() => {
    setPage(1);
  }, [search, modelFilter, shiftFilter, pageSize]);
        ...
        {hasData && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <StatCard
              icon={FiGrid}
              label="Components Produced"
              value={totals.componentQty.toLocaleString()}
              tone="violet"
            />
            <StatCard
              icon={FiPackage}
              label="Sheets"
              value={totals.actualQty.toLocaleString()}
              tone="blue"
            />
            <StatCard
            ...
            <StatCard
              icon={FiClock}
              label="Downtime"
              value={`${totals.totalDowntimeMins} min`}
              tone="amber"
            />
            <StatCard
              icon={FiActivity}
      ...
      <div className="flex-1 overflow-auto p-3 sm:p-4 flex flex-col gap-3">
        ...
        <SectionCard className="p-4 shrink-0">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="flex gap-2 pb-0.5 shrink-0 flex-wrap">
              ...
              >
                {loading ? <Spinner /> : <FiSearch className="w-3.5 h-3.5" />}{" "}
        ...
        <div className="flex items-center justify-between gap-3 shrink-0 flex-wrap">
          <div className="flex gap-1 bg-white rounded-xl border border-slate-200 shadow-sm p-1 flex-wrap">
            {[["summary", FiBarChart2, "Summary"]].map(
              ([mode, Icon, label]) => (
                <button
                  key={mode}
          ...
          {hasData && (
            <div className="flex items-center gap-2 flex-wrap">
              <button
                onClick={() => exportCSV(sortedRows, visibleColumns)}
            ...
            <div className="flex items-center justify-between gap-3 px-3 py-2.5 border-b border-slate-100 flex-wrap">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search SAP code / description…"
                  ...
                  {colMenuOpen && (
                    <div className="absolute right-0 mt-1 z-30 bg-white border border-slate-200 rounded-lg shadow-lg p-2 w-56 max-h-72 overflow-auto">
                      {columns.map((c) => (
                        <label
                          key={c.key}
                          className={`flex items-center gap-2 px-2 py-1 text-xs rounded hover:bg-slate-50 ${c.always ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                        >
                          <input
                            type="checkbox"
                            disabled={c.always}
              ...
              <table className="border-separate border-spacing-0 w-full">
                ...
                <tbody>
                  {isAnyLoading ? (
                    ...
                    pagedRows.map((r) => (
                      <tr
                        key={`${r.dateFrom}|${r.shift}|${r.sapCode}|${r.srNo}`}
                        className={`hover:bg-blue-50/30 transition-colors even:bg-slate-50/30 ${r.isGhost ? "bg-slate-50/60" : ""}`}
                      >
                        {visibleColumns.map((c) => (
                          <td
                            key={c.key}
                            className={`px-2.5 py-2.5 border-b border-r border-slate-100 ${c.align === "center" ? "text-center" : ""}`}
                          >
