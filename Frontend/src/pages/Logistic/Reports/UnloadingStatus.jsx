...
const UnloadingStatus = () => {
  ...
  const stats = useMemo(() => {
    ...
    const labelScanned = data.filter(
      (r) => r.FG_LabelPrinting === SCANNED,
    ).length;
    const autoScanned = data.filter((r) => r.FG_Auto_Scan === SCANNED).length;
    const vehicles = new Set(data.map((r) => r.Vehicle_No).filter(Boolean))
      .size;
  ...
  const filteredData = useMemo(() => {
    let d = data;
    if (scanFilter.value === "label")
      d = d.filter((r) => r.FG_LabelPrinting === SCANNED);
    else if (scanFilter.value === "auto")
      d = d.filter((r) => r.FG_Auto_Scan === SCANNED);
    else if (scanFilter.value === "unloading")
      d = d.filter((r) => r.FG_Unloading === SCANNED);
    else if (scanFilter.value === "missing_label")
      d = d.filter((r) => r.FG_LabelPrinting !== SCANNED);
    else if (scanFilter.value === "missing_auto")
      d = d.filter((r) => r.FG_Auto_Scan !== SCANNED);
    else if (scanFilter.value === "missing_unloading")
      d = d.filter((r) => r.FG_Unloading !== SCANNED);
      ...
      <div className="flex-1 min-h-0 overflow-hidden p-5 flex flex-col gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm shrink-0">
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            {/* All action buttons — Query, quick filters, Export, Clear — inline */}
            <button
              onClick={handleQuery}
              disabled={anyLoading}
              className="px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2 shadow-md shadow-blue-200 cursor-pointer"
            >
              {loading ? (
                <FiLoader size={13} className="animate-spin" />
              ) : (
            ...
            <button
              ...
              disabled={anyLoading}
              className="px-4 py-2 rounded-lg text-white text-sm font-semibold transition-all active:scale-95 disabled:opacity-50 flex items-center gap-1.5 shadow-sm bg-amber-500 hover:bg-amber-600 cursor-pointer"
            >
              {ydayLoading && <FiLoader size={13} className="animate-spin" />}
              Yesterday
            </button>
            ...
            <button
              onClick={handleToday}
              disabled={anyLoading}
              className="px-4 py-2 rounded-lg text-white text-sm font-semibold transition-all active:scale-95 disabled:opacity-50 flex items-center gap-1.5 shadow-sm bg-emerald-600 hover:bg-emerald-700 cursor-pointer"
            >
              {todayLoading && <FiLoader size={13} className="animate-spin" />}
              Today
            </button>
            ...
            <button
              onClick={handleMTD}
              disabled={anyLoading}
              className="px-4 py-2 rounded-lg text-white text-sm font-semibold transition-all active:scale-95 disabled:opacity-50 flex items-center gap-1.5 shadow-sm bg-indigo-600 hover:bg-indigo-700 cursor-pointer"
            >
              {monthLoading && <FiLoader size={13} className="animate-spin" />}
              MTD
            </button>
            ...
            <button
              onClick={handleClear}
              className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-300 text-gray-400 hover:text-red-500 text-sm transition-all flex items-center gap-1.5 cursor-pointer"
            >
              <FiX size={13} /> Clear
            </button>
          </div>
                  ...
                  <tbody>
                    {filteredData.map((row, i) => (
                      ...
                      >
                        ...
                        <td
                          className="px-4 py-2.5 text-gray-700 font-medium whitespace-nowrap max-w-[160px] truncate"
                        ...
                        <td className="px-4 py-2.5">
                          <ScanBadge status={row.FG_LabelPrinting} />
                        </td>
                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">
                          {fmtDisplay(row.FG_LabelPrinting_Date)}
                        </td>

                        <td className="px-4 py-2.5">
                          <ScanBadge status={row.FG_Auto_Scan} />
                        </td>
                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">
                          {fmtDisplay(row.FG_Auto_Scan_Date)}
                        </td>

                        {/* FG_Unloading is always 'SCANNED' per the SQL */}
                        <td className="px-4 py-2.5">
                          <ScanBadge status={row.FG_Unloading} />
                        </td>
                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">
                          {fmtDisplay(row.FG_Unloading_Date)}
                        </td>
                        ...
                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">
                          {fmtDisplay(row.Vehicle_Entry_Time)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                          ...
                          >
                            ...
                            <td className="px-5 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                />
                                <span
                                  className="text-gray-700 text-xs font-medium truncate max-w-[200px]"
                                  title={item.key}
                                >
                                  {item.key}
                                </span>
            ...
            <div className="flex-1 overflow-auto">
              {data.length === 0 ? (
                ...
                (() => {
                  ...
                  const rows = Object.values(vehicleMap).sort(
                    (a, b) => b.total - a.total,
                  );
                  return (
                    <table className="w-full text-xs">
                        ...
                        {rows.map((r, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">
                                {fmtDisplay(r.Vehicle_Entry_Time)}
                              </td>
                              ...
                              <td className="px-4 py-2.5 text-center text-violet-600 font-medium">
                                {r.autoOk}
                              </td>
                              <td className="px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-xs font-semibold w-9 text-right ${lPct < 80 ? "text-red-500" : "text-emerald-600"}`}
                                  >
                                    {lPct}%
                                  </span>
                                </div>
                              ...
                              <td className="px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-xs font-semibold w-9 text-right ${aPct < 80 ? "text-red-500" : "text-emerald-600"}`}
                                  >
                                    {aPct}%
                                  </span>
                                </div>
